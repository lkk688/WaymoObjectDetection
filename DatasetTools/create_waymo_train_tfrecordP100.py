...
def loadWaymoFrames(PATH):
    ...
    train_folders = ["training_0010"]#,"training_0006"]
...
#         difficult_obj = []
        ...
        for camera_labels in frame.camera_labels: #5 cameras
            ...
            for label in camera_labels.labels:                    
                ...
                widthscale=label.box.length/image_width
                hightscale=label.box.width/image_height
                if xmin<=xmax and ymin<=ymax and xmin>=0 and ymin>=0 and widthscale>0.01 and hightscale>0.01:# and xmax<=image_width and ymax<=image_height:# and area>2000:
                    if xmax>image_width:
                        xmax=image_width
                        print("xmax overflow")
...
def main(_):
    ...
    _create_tf_record_from_waymo_annotations('/DATA5T/Dataset/WaymoDataset/',
                                            '/DATA5T/Dataset/WaymoTFRecord/train10/TFRecordTrain-',
                                             label_map_dict,
                                            num_shards)
