...
def convertafilteredimageallobject(outputpath, allfiles_len, alltrainfiles_len, trainsize, valsize, stepsize, prefix):
    images = []
    annotations = []
    smallannotations = []
    valimages = []
    ...
    for fileidx in range(allfiles_len):
        ...
        for imagedict in image_dictarraylist:
            ...
            if stepcheck ==0 : #cameraname=='1' and 
    ...
    json_out_dir = outputpath #'/DATA5T/Dataset/WaymoCOCO/'#'/data/cmpe249-f20/WaymoCOCOMulti/trainvalall'
    json_out_dir= Path(json_out_dir)
    trainannotationfile = "annotations_train"+str(trainsize)+prefix+".json"#"filteredbig.json"
    count = 0
    with (json_out_dir / trainannotationfile).open('w') as f:
        for i, anno in enumerate(annotations):
            anno['id'] = i #count #i #set as image frame ID
    ...
    count=0
    valannotationfile = "annotations_val"+str(valsize)+prefix+".json"#"filteredbig.json"
    with (json_out_dir / valannotationfile).open('w') as f:
        for i, anno in enumerate(valannotations):
            anno['id'] = i #count #i #set as image frame ID, unique to all other annotations in the dataset
            count = count +1
...
if __name__ == "__main__":
    ...
    output_dir="/data/cmpe249-f20/WaymoCOCOMulti/trainvalall/"#"/DATA5T/Dataset/WaymoCOCO/pickles"

    create_partial_data = True
    step_size=8
    prefix='step8allobject'
    if create_partial_data:
        ...
        convertafilteredimageallobject(output_dir, allfiles_len, alltrainfiles_len, alltrainfiles_len, allfiles_len-alltrainfiles_len, step_size, prefix)
