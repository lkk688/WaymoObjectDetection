...
#             glob(join(self.load_dir, '*.tfrecord')))
    ...
    async def concurrenttaskthread(self):#https://github.com/jersobh/python-parallelism-examples/blob/master/async_concurrent_futures.py
...
#             self.convert_one(file_idx)
    ...
    async def convert_one_async(self, file_idx):
...
#             loop.close()
    ...
    async def savetofiles(data, file_idx, frame_idx):
        frame = open_dataset.Frame()
        frame.ParseFromString(bytearray(data.numpy()))
        await self.save_image(frame, file_idx, frame_idx)
        await self.save_calib(frame, file_idx, frame_idx)
        await self.save_lidar(frame, file_idx, frame_idx)
        await self.save_pose(frame, file_idx, frame_idx)

        if not self.test_mode:
            await self.save_label(frame, file_idx, frame_idx)
...
def parsesinglerecord(data):
    print("Detail:",data)
    return data


def parseWaymo_tfrecord(record):
    c_start = time.time()
    #print("Current file:",record)
    #filepath=record.numpy()
    dataset = tf.data.TFRecordDataset(record, compression_type='')
    frame = open_dataset.Frame()
    frame.ParseFromString(bytearray(dataset.numpy())) #.numpy()
    
    #dataset.map(parsesinglerecord)
    # frames=[]
    # for frame_idx, data in enumerate(dataset):
    #     print(frame_idx)
    #     frame = open_dataset.Frame()
...
if __name__ == "__main__":
    if tf.executing_eagerly():
        print("TF in eager mode")

    folders = ['training_0003']#, 'training_0001']
    root_path="/data/cmpe249-f20/Waymo/"
    out_dir="/data/cmpe249-f20/WaymoKittitTest1"
    workers=4
    batch_size=8
    ...
    list_ds = tf.data.Dataset.list_files(glob(os.path.join(root_path, folders[0], "*.tfrecord")), shuffle=False)
    print("dataset size:", tf.data.experimental.cardinality(list_ds).numpy())
    print(list(list_ds.as_numpy_iterator()))

    #AUTOTUNE = tf.data.AUTOTUNE
    list_ds_frames = list_ds.map(parseWaymo_tfrecord, num_parallel_calls=8)


    ds = list_ds_frames.batch(batch_size)
    for f in ds.take(2):
        print(len(f))
    ...
    #     converter.convert_multithread()#convertcoroutine()#concurrenttaskthread()#.convert()