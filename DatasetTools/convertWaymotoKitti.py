...
import os
import Waymo2KittiAsync#Waymo2Kitti
...
c_start = time.time()
print(c_start)
save_dir = osp.join(out_dir, 'training', 'training_0001')#put to the same folder
...
converter = Waymo2KittiAsync.Waymo2KITTIAsync(
        ...
        workers=workers,
        test_mode=False)
converter.concurrenttaskthread()
print(f"Finished, Execution time: { time.time() - c_start }")
#converter.convert_multithread()#using multiple process
#converter.convert()
